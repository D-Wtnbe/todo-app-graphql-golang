...
type TaskQuery struct {
	...
	withNamedAssignedTo map[string]*UserQuery
...
func (tq *TaskQuery) QueryAssignedTo() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, task.AssignedToTable, task.AssignedToPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (tq *TaskQuery) WithAssignedTo(opts ...func(*UserQuery)) *TaskQuery {
	query := (&UserClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withAssignedTo; query != nil {
		if err := tq.loadAssignedTo(ctx, query, nodes,
			func(n *Task) { n.Edges.AssignedTo = []*User{} },
			func(n *Task, e *User) { n.Edges.AssignedTo = append(n.Edges.AssignedTo, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedAssignedTo {
		if err := tq.loadAssignedTo(ctx, query, nodes,
			func(n *Task) { n.appendNamedAssignedTo(name) },
			func(n *Task, e *User) { n.appendNamedAssignedTo(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TaskQuery) loadAssignedTo(ctx context.Context, query *UserQuery, nodes []*Task, init func(*Task), assign func(*Task, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.AssignedToTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(task.AssignedToPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(task.AssignedToPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.AssignedToPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "assigned_to" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
// WithNamedAssignedTo tells the query-builder to eager-load the nodes that are connected to the "assigned_to"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithNamedAssignedTo(name string, opts ...func(*UserQuery)) *TaskQuery {
	query := (&UserClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedAssignedTo == nil {
		tq.withNamedAssignedTo = make(map[string]*UserQuery)
	}
	tq.withNamedAssignedTo[name] = query
	return tq
}
