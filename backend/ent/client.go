...
import (
	...
	"github.com/D-Wtnbe/todo-app-graphql-golang/ent/migrate"
...
// QueryAssignedTo queries the assigned_to edge of a Task.
func (c *TaskClient) QueryAssignedTo(t *Task) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := t.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Project, Task, Todo, User []ent.Hook
	}
	inters struct {
		Project, Task, Todo, User []ent.Interceptor
	}
)
